(function($){
    ...
    App.module.controller("entry", function($scope, $rootScope, $http, $timeout, Contentfields){
        ...
        $scope.loadVersions = function() {
            ...
            $http.post(App.route("/api/landingpages/getVersions"), {"id":$scope.entry["_id"], "colId":$scope.collection["_id"]}).success(function(data){
        ...
        $scope.clearVersions = function() {
            ...
            App.Ui.confirm(App.i18n.get("Are you sure?"), function(){
                ...
                $http.post(App.route("/api/landingpages/clearVersions"), {"id":$scope.entry["_id"], "colId":$scope.collection["_id"]}).success(function(data){
                    $timeout(function(){
                        $scope.versions = [];
        ...
        $scope.restoreVersion = function(versionId) {
            ...
            App.Ui.confirm(App.i18n.get("Are you sure?"), function(){
                ...
                $http.post(App.route("/api/landingpages/restoreVersion"), {"docId":$scope.entry["_id"], "colId":$scope.collection["_id"],"versionId":versionId}).success(function(data){

                    setTimeout(function(){
                        msg.close();
                        location.href = App.route("/landingpages/entry/"+$scope.collection["_id"]+'/'+$scope.entry["_id"]);
                    }, 1000);
                }).error(App.module.callbacks.error.http);
            });
        };
        ...
        $scope.save = function(){
            ...
            if ($scope.validateForm(entry)) {
                $http.post(App.route("/api/landingpages/saveentry"), {"collection": collection, "entry":entry, "createversion": true}).success(function(data){
