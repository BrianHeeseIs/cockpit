@start('header')
    ...
    {{ $app->assets(['landingpages:assets/landingpages.js','landingpages:assets/js/entries.js'], $app['cockpit/version']) }}
...
<div data-ng-controller="entries" ng-cloak>
        ...
        <div class="uk-navbar-content" data-ng-show="collection && collection.count">
            <form class="uk-form uk-margin-remove uk-display-inline-block" method="get" action="?nc={{ time() }}">
                <div class="uk-form-icon">
                    ...
                    <a class="uk-text-small" href="@route('/landingpages/entries/'.$collection['_id'])" data-ng-show="filter"><i class="uk-icon-times"></i> @lang('Reset filter')</a>
                </div>
            </form>
        </div>
        @endif
    ...
    <div class="app-panel uk-margin uk-text-center" data-ng-show="entries && !filter && !entries.length">
        ...
        <a href="@route('/landingpages/entry/'.$collection["_id"])" class="uk-button uk-button-success uk-button-large">@lang('Add entry')</a>
    </div>
                            ...
                            <td>
                                <div class="uk-grid uk-grid-preserve uk-text-small" data-ng-repeat="field in fields">
                                    ...
                                    <div class="uk-width-medium-4-5">
                                        <a class="uk-link-muted" href="@route('/landingpages/entry/'.$collection["_id"])/@@ entry._id @@">@@ entry[field.name] @@</a>
                                    </div>
                                </div>
                            </td>
                                ...
                                <div data-uk-dropdown>
                                    ...
                                    <div class="uk-dropdown uk-dropdown-flip uk-text-left">
                                        <ul class="uk-nav uk-nav-dropdown uk-nav-parent-icon">
                                            <li><a href="@route('/landingpages/entry/'.$collection["_id"])/@@ entry._id @@"><i class="uk-icon-pencil"></i> @lang('Edit entry')</a></li>
